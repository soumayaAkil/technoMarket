...
class Techmarket_Breadcrumb {
	...
	private function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'techmarket' ) );
	}
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Posts tagged &ldquo;%s&rdquo;', 'techmarket' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Author: %s', 'techmarket' ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( esc_html__( 'Search results for &ldquo;%s&rdquo;', 'techmarket' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( esc_html__( 'Page %d', 'techmarket' ), get_query_var( 'paged' ) ) );
		}
	}
}